...
public class JDGetOrderInfoNoAgentJob implements Job {
	private final static String EXPRESS_ID="JD";
	private static Logger log = LoggerFactory
			.getLogger(JDGetOrderInfoNoAgentJob.class);
	...
	private void getOrderInfo() throws JdException {
		...
		while (iter.hasNext()) {
			...
			if(Integer.parseInt(orderResponse.getCode())>0) {
				if(Integer.parseInt(orderResponse.getCode())==73) { //该api是增值api，请将您的app入住云鼎平台方可调用
					int i=WmsServ.updateWmsSoOrderPay(soNo, "1-货到付款");
					if (i != 1) {
				...
				log.error("---京东API平台调用错误，错误码为【"+orderResponse.getCode()+"】【"
						+Thread.currentThread().getStackTrace()[1].getMethodName()+"】，SO订单号为【" + soNo + "】，京东订单号为【" + sourceOrderNo + "】---"
						+ CommonUtil.curDate());
				continue;
			}
